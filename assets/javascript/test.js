        ...
        displayQuestion: function(){
            ...
            if (quizCounter < this.Q.length) {
                ...
                for (let i = 0; i < this.C.length; i++) {
                    $(".choices").append("<button>"+this.C[quizCounter][i]+"</button><br>");
...
let displayResults = function() {
    $("body").html(`<div class = Quiz>
                <h1 class = title>Trivia Quiz!</h1>`+"<div class = results><h2>You scored: "+parseInt(((Quiz.Q.length-(incorrectAnswerTally+unansweredTally))/Quiz.Q.length)*100)+"%</h2></div>"+"<div class = results> Correct Answers: " + correctAnswerTally + "</div>" + "<div class = results> Incorrect Answers: " + incorrectAnswerTally + "</div>" + "<div class = results> Unanswered: " + unansweredTally + "</div>"+"<div class = reset><button>Start Over?</button>");
};
